...
- **Timeline markers** — a short label on any frame for navigation or a to-do note, added from the
  ...
  exists. Tap to jump, tap again to rename or delete, drag to move; with the playhead on a marker the
  timeline bar's marker button deletes it instead of adding. Markers follow inserted and
...
- A **group** can be faded as one thing as well — member opacity × group opacity. The **Group**
  slider sits on the group header in the layer panel (even when collapsed); Animate from the
  group's own row on the timeline bar, same as Animate group transform
...
- A play range set with In/Out shows as two thin markers running the full height of the timeline — drag either one to adjust it, tap one to jump there. It bounds playback and the PNG sequence / video exports
...
- **Export dialog** — pick a format (PNG sequence, PNG frame, PSD frame, MP4, WebM via [mediabunny](https://github.com/Vanilagy/mediabunny)), then set what applies to it: size (100% / 50% / 25% — PSD always exports at full size), frame range (all frames, the play In-Out range, or a typed custom span), and video quality (MP4/WebM)
...
npm test           # Vitest — pure-logic unit tests (1402); canvas/DOM code isn't node-testable
