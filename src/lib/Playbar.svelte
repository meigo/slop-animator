<script lang="ts">
  import { state as appState, bump, playbackController, setAnimationLength, setPlayRangeIn, setPlayRangeOut, clearPlayRange } from "../state/appState.svelte";
...
<div class="flex items-center gap-2 p-2 border-t border-border bg-surface text-text text-sm">
  ...
  <div class="flex items-center gap-1">
    ...
    <button class={btn} title="Previous frame" onclick={() => go(appState.playhead - 1)}><ChevronLeft size={16} /></button>
    ...
    <button class={btn} title="Next frame" onclick={() => go(appState.playhead + 1)}><ChevronRight size={16} /></button>
    <button class={btn} title="Last frame" onclick={() => go(appState.project.frameCount - 1)}><SkipForward size={16} /></button>
  </div>

  <span class="text-text-secondary">Frame {appState.playhead + 1}/{appState.project.frameCount}</span>
  <label class="flex items-center gap-1 text-text-secondary">Length
    <input class="w-14 bg-surface border border-border text-text px-1" type="number" min="1" max="9999"
           value={appState.project.frameCount} onchange={commitLength} />
  </label>
        ...
        <div class="absolute right-0 bottom-full mb-2 z-30 w-48 p-3 rounded-lg bg-surface border border-border shadow-md flex flex-col gap-2 text-xs">
          <div class="flex items-center gap-2">
            ...
            <input class="w-12 bg-surface border border-border text-text px-1" type="number" min="1" max="60"
                   value={appState.project.fps} onchange={(e) => setFps(+e.currentTarget.value)} />
            <div class="flex gap-px ml-auto">
              {#each FPS_PRESETS as p}
                <button class="px-1.5 py-0.5 rounded" class:bg-surface-active={appState.project.fps === p}
                        onclick={() => setFps(p)}>{p}</button>
              {/each}
            </div>
          </div>
          <label class="flex items-center gap-2"><input type="checkbox" bind:checked={appState.playback.loop} /> <span class="text-text-secondary">Loop playback</span></label>
        </div>
      {/if}
    </div>
  </div>
